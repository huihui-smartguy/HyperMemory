...
# 启用的模块列表（逗号分隔）。生产默认仅暴露 vault + dev。
# 可选：vault,analytics,evolution,cognitive-graph,retrieval-xray,dev
ARG NEXT_PUBLIC_ENABLED_MODULES=vault,dev
...
ENV NEXT_PUBLIC_ENABLED_MODULES=$NEXT_PUBLIC_ENABLED_MODULES
